...
def build_retriever():
    """Build the retriever using the winning Lab 3 configuration.

    This uses sliding window chunks of size 800 (default Lab 3 best setting) and a
    dense retriever over those chunks.
    """
    corpus = load_corpus()
    # Use sliding window chunks (size=800) – Lab 3 best performing configuration.
...
def judge_faithfulness(answer_text: str, context: str) -> int:
    """Judge faithfulness of an answer against the provided context.

    Returns 1 for faithful, 0 otherwise. Handles partial refusals by treating a
    correct refusal as faithful.
    """
    # Quick check for correct refusal
    if answer_text.strip().startswith(REFUSAL):
        # If the gold answer expects a refusal, treat as faithful (score 1)
        return 1
    verdict = llm_judge(
        JUDGE_RUBRIC_FAITHFULNESS.format(context=context[:8000], answer=answer_text),
        tier="LARGE",
    )
...
def judge_correctness(question: str, candidate: str, reference: str) -> int:
    """Judge correctness of a candidate answer against a gold reference answer.

    Returns 2 for fully correct, 1 for partially correct, 0 for incorrect.
    Handles refusal cases explicitly: if the reference expects a refusal (contains
    the word "REFUSE"), a matching refusal scores 2.
    """
    # Detect if reference expects a refusal
    expects_refusal = "REFUSE" in reference.upper()
    candidate_is_refusal = candidate.strip().startswith(REFUSAL)
    if expects_refusal:
        if candidate_is_refusal:
            return 2
        # Non-refusal when refusal expected -> treat as incorrect (score 0)
        return 0
    # Normal case: use the LLM judge rubric
    verdict = llm_judge(
        JUDGE_RUBRIC_CORRECTNESS.format(question=question, reference=reference, candidate=candidate),
        tier="LARGE",
    )
    return int(verdict.get("score", 0))
